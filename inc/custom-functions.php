...
function book_landing_page_change_comment_form_default_fields( $fields ){
    ...
    $fields['author'] = '<p class="comment-form-author"><label class="screen-reader-text" for="author">' . esc_html__( 'Name*', 'book-landing-page' ) . '</label><input id="author" name="author" placeholder="' . esc_attr__( 'Name*', 'book-landing-page' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['email'] = '<p class="comment-form-email"><label class="screen-reader-text" for="email">' . esc_html__( 'Email*', 'book-landing-page' ) . '</label><input id="email" name="email" placeholder="' . esc_attr__( 'Email*', 'book-landing-page' ) . '" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';
...
function book_landing_page_change_comment_form_defaults( $defaults ){
    ...
    $defaults['comment_field'] = '<p class="comment-form-comment"><label class="screen-reader-text" for="comment">' . esc_html__( 'Comment', 'book-landing-page' ) . '</label><label for="comment"></label><textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Comment', 'book-landing-page' ) . '" cols="45" rows="8" aria-required="true"></textarea></p>';
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    ...
                    <p><?php printf( __( '%1$s is now installed and ready to use. Click below to see theme documentation, plugins to install and other details to get started.', 'book-landing-page' ), $name ) ; ?></p>
...
if( ! function_exists( 'book_landing_page_restore_admin_notice' ) ) :
/**
 * Restoring admin notice on theme switch
 */
function book_landing_page_restore_admin_notice(){
    update_option( 'book_landing_page_admin_notice', false );
}
endif;
add_action( 'after_switch_theme', 'book_landing_page_restore_admin_notice' );